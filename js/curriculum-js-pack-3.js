  ...
  t.modules.push(
    ...
    /* ---------- JS 0x09 ---------- */
    {
      ...
      theory: J(
        ...
        "A **Set** is a collection where every value can appear **only once**. Add the same value twice and the",
        "second add is silently ignored. **Why?** It answers two questions fast: *is this value already here?* and",
        "*give me these values with the duplicates stripped out.*",
        "Build a Set from an array to dedupe it, then spread (`...`) it back into a plain array when you need one.",
        "~~~js",
        ...
        "~~~",
        "> WARNING — A Set stores **values only**, not key/value pairs. `s.add('a')` puts in the value `'a'`; there",
        "is no `s.set(...)` and no `s.get(...)`. If you need to look something up *by* a key, you want a Map.",
        "## Map — a key → value store",
        "A **Map** links each **key** to a **value**, like a tiny lookup table. It's similar to a plain object, but",
        "with two superpowers: keys can be **any type** (numbers, objects, even functions — not just strings), and it",
        "**remembers the order** you inserted things in. Use `.set(key, value)` to store, `.get(key)` to read.",
        "~~~js",
        "const m = new Map();",
        ...
        "~~~",
        "> WARNING — Map methods are **not** array/object syntax: it's `m.set(k, v)` and `m.get(k)`, never `m[k] = v`.",
        "Writing `m['a'] = 1` puts a normal property on the object instead — `m.get('a')` stays `undefined` and",
        "`m.size` doesn't change.",
        "## Which one, and when",
        "Reach for a **Set** when you only care *whether* a value is present (dedupe, membership, overlap). Reach for",
        "a **Map** when you need to attach data *to* each key (counting how often something appears, looking values up).",
        "> INTEL — `new Set(b).has(x)` is the fast membership test that powers intersections and 'is it in the list?'.",
        "Checking a Set is far quicker than scanning a whole array with `arr.includes(x)` every time."
      ),
      exercises: [
        {
    ...
    /* ---------- JS 0x0A ---------- */
    {
      ...
      theory: J(
        ...
        "A **class** is a blueprint for making objects that share the same shape and abilities. It bundles **state**",
        "(the data each object remembers) with **behaviour** (the things it can do, called **methods**). **Why?** When",
        "you need many objects of the same kind — drones, players, stacks — you write the blueprint once and stamp out",
        "as many as you want, each keeping its own data.",
        "Two key parts: the **constructor** runs once when the object is born and sets up its starting state; the other",
        "functions inside the class are its **methods**.",
        "~~~js",
        "class Drone {",
        ...
        "~~~",
        "## new — build an instance",
        "**new** is what turns the blueprint into a real object (an **instance**). It creates a fresh object, runs the",
        "constructor on it, and hands it back to you.",
        "> WARNING — Always use **new**. Calling `Drone('x')` *without* `new` doesn't build an object — in a class it",
        "throws an error, and even where it doesn't, `this` won't be your new instance. The fix is always `new Drone('x')`.",
        "## this — the current instance",
        "Inside a method, **this** refers to the specific object the method was called on. You read and write that",
        "object's own data through `this.` — like `this.name` or `this.alt`. Every object made with `new` carries its",
        "**own** copy of the state, so changing one instance never touches another.",
        "~~~js",
        "const a = new Drone('A');",
        "const b = new Drone('B');",
        "a.climb(10);        // only a.alt changes; b.alt is still 0",
        "~~~",
        "> WARNING — Inside a method you must write `this.name`, not just `name`. Bare `name` looks for a separate",
        "variable (and usually errors or reads the wrong thing); only `this.name` reaches the instance's own data.",
        "> INTEL — Default parameters work in constructors too: `constructor(start = 0) { this.n = start; }` — then",
        "`new Counter()` starts at 0 while `new Counter(10)` starts at 10."
      ),
      exercises: [
        {
