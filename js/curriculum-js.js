  ...
  window.registerTrack({
    ...
    modules: [
      ...
      /* ---------- JS 0x01 ---------- */
      {
        ...
        theory: J(
          "## Jacking in",
          "JavaScript runs everywhere — browsers, servers, even spaceships. Push output to the console with **console.log()**.",
          "~~~js",
          "console.log('Wake up, Neo...');",
          "~~~",
          "## Variables",
          "Declare with **let** (reassignable) or **const** (locked). Reach for `const` unless you must reassign.",
          "~~~js",
          "const codename = 'Lain';",
          "let clearance = 7;",
          "~~~",
          "## Template literals",
          "Backtick strings interpolate with ${...} — far cleaner than + concatenation.",
          "~~~js",
          "const name = 'Faye';",
          "console.log(`${name} :: clearance L${clearance}`);",
          "~~~",
          "## Types & equality",
          "**typeof x** reports 'string', 'number', 'boolean', 'object', 'function', 'undefined'.",
          "> WARNING — Always use === (strict), not == . Loose == coerces types and causes ghosts in the machine."
        ),
        exercises: [
          {
      ...
      /* ---------- JS 0x02 ---------- */
      {
        ...
        theory: J(
          "## Arithmetic",
          "`+ - * /` plus **%** (remainder) and **\\*\\*** (power). `n % 2` tests even/odd.",
          "## Comparison & booleans",
          "Strict comparisons `=== !== < <= > >=` return booleans. Combine with **&&** (and), **||** (or), **!** (not).",
          "~~~js",
          "const ok = level >= 0 && level <= 100;",
          "~~~",
          "## Branching",
          "**if / else if / else**. An early `return` inside a branch exits the function immediately.",
          "~~~js",
          "if (n % 15 === 0) return 'SYNCHRONIZED';",
          "else if (n % 3 === 0) return 'MELCHIOR';",
          "~~~",
          "> INTEL — Check the most specific condition first (divisible-by-15 before 3 or 5)."
        ),
        exercises: [
          {
      ...
      /* ---------- JS 0x03 ---------- */
      {
        ...
        theory: J(
          "## Strings",
          "Strings are immutable. Useful methods: `.toUpperCase()`, `.toLowerCase()`, `.includes(x)`, `.split(sep)`, `.slice(a,b)`.",
          "## Arrays",
          "Ordered, mutable lists. Transform without loops using **.map()**, **.filter()**, **.reduce()**.",
          "~~~js",
          "[1,2,3,4].filter(function (n) { return n % 2 === 0; });  // [2, 4]",
          "['a','b'].join('-');                                     // 'a-b'",
          "~~~",
          "## Split / reverse / join",
          "Turn a string into an array, transform, and rejoin.",
          "~~~js",
          "'LAIN'.split('').reverse().join('');  // 'NIAL'",
          "~~~",
          "> INTEL — `str.split('')` explodes a string into a character array you can map/filter."
        ),
        exercises: [
          {
      ...
      /* ---------- JS 0x04 ---------- */
      {
        ...
        theory: J(
          "## Arrow functions",
          "A compact function syntax. These two are equivalent:",
          "~~~js",
          "const dbl = function (x) { return x * 2; };",
          "const dbl2 = (x) => x * 2;",
          "~~~",
          "## reduce",
          "Fold an array into a single value — the accumulator pattern, built in.",
          "~~~js",
          "[10,20,12].reduce((a, c) => a + c, 0);  // 42",
          "~~~",
          "## Objects",
          "Key/value maps. Read with `obj[key]`; test a key with `key in obj`.",
          "~~~js",
          "const freq = {};",
          "freq['a'] = (freq['a'] || 0) + 1;",
          "~~~",
          "> INTEL — `(freq[k] || 0) + 1` is the classic one-liner for building a frequency table."
        ),
        exercises: [
          {
