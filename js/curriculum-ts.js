  ...
  window.registerTrack({
    ...
    modules: [
      ...
      /* ---------- TS 0x01 ---------- */
      {
        ...
        theory: J(
          '## Static types',
          'TypeScript is JavaScript that you **annotate**. The compiler reads the annotations, hunts for contradictions, then strips them away - the code that runs is plain JS.',
          'Annotate a binding with `: type`. Primitives are `number`, `string`, `boolean`.',
          '~~~ts',
          ...
          '~~~',
          '## Typed functions',
          'Annotate each parameter and the return value. Now the compiler enforces the contract at every call site.',
          '~~~ts',
          ...
          '~~~',
          '## Literal & union types',
          'A type can be an exact **literal** value, and a **union** (`|`) accepts any one of several types - perfect for a fixed menu of options.',
          '~~~ts',
          ...
          '}',
          '~~~',
          '> INTEL - The annotations vanish at runtime. They exist to brief the compiler, not the CPU.'
        ),
        exercises: [
          {
      ...
      /* ---------- TS 0x02 ---------- */
      {
        ...
        theory: J(
          ...
          'Describe the **shape** of an object once, then reuse it everywhere. An `interface` and an object `type` are nearly interchangeable here.',
          '~~~ts',
          'interface Operator {',
          '  name: string;',
          '  clearance: number;',
          ...
          '~~~',
          '## Typed arrays',
          'A list of one type is `T[]`. The compiler now guards every element.',
          '~~~ts',
          'const cells: number[] = [10, 20, 12];',
          "const crew: string[] = ['Spike', 'Jet', 'Faye'];",
          '~~~',
          '## Tuples',
          'A **tuple** is a fixed-length array where each slot has its own type - great for coordinates or paired returns.',
          '~~~ts',
          'const point: [number, number] = [128, 256];',
          '~~~',
          '## Optional parameters',
          'A `?` makes a parameter optional; inside the function its type includes `undefined`, so supply a fallback.',
          '~~~ts',
          'function greet(name: string, rank?: string): string {',
          "  return rank ? `${rank} ${name}` : name;",
          '}',
          '~~~',
          '> INTEL - `param?: T` is shorthand for `param: T | undefined`. Always handle the missing case.'
        ),
        exercises: [
          {
      ...
      /* ---------- TS 0x03 ---------- */
      {
        ...
        theory: J(
          '## Why generics',
          'Sometimes a function works on **any** type but must preserve it. Without generics you reach for `any` and lose all safety. A **type parameter** `<T>` keeps the link between input and output.',
          '~~~ts',
          ...
          '~~~',
          '## Generic over arrays',
          'A type parameter shines on collections - the element type flows straight through.',
          '~~~ts',
          ...
          '~~~',
          '## Inference',
          'You rarely write `<T>` at the call site - the compiler **infers** it from the argument.',
          '~~~ts',
          'firstOf([1, 2, 3]); // T inferred as number',
          '~~~',
          '> INTEL - Generics are a contract between caller and function: "give me a T, I will give you back something built from that same T".'
        ),
        exercises: [
          {
