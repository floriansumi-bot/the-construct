...
(function () {
  ...
  t.modules.push(
    ...
    /* ---------- TS 0x0C ---------- */
    {
      ...
      theory: J(
        "## Throw on bad input",
        "**WHAT** — `throw new Error('...')` aborts the function immediately and hands a typed error up the call stack.",
        "**WHY** — bad input should fail loudly, not slip through as a wrong number that corrupts everything downstream.",
        "~~~ts",
        ...
        "}",
        "~~~",
        "## Catch what others throw",
        "**WHAT** — wrap risky calls in `try { ... } catch (e) { ... }` so a thrown error becomes a value you can handle.",
        "**WHY** — without `catch`, a single throw crashes the whole program; with it, you recover or report gracefully.",
        "~~~ts",
        "try {",
        "  const n = toInt(raw);",
        "} catch (e) {",
        ...
        "}",
        "~~~",
        "> WARNING — in `catch (e)` the error is typed `unknown` — narrow it with `e instanceof Error` before touching `.message`.",
        "## The typed Result pattern",
        "**WHAT** — a **Result** union, `{ ok: true; value: T } | { ok: false; error: string }`, returns failure as data.",
        "**WHY** — instead of throwing, you RETURN the failure, so the caller cannot reach `.value` without first checking `.ok`.",
        "~~~ts",
        ...
        "}",
        "~~~",
        "> INTEL — the union forces `if (r.ok)` before `r.value` compiles — failure becomes impossible to ignore."
      ),
      exercises: [
        {
    ...
    /* ---------- TS 0x0D ---------- */
    {
      ...
      theory: J(
        ...
        "**WHAT** — `split('')` explodes a string into a character array; you transform it, then `join('')` rebuilds a string.",
        "**WHY** — strings are immutable, so array methods like `reverse()` and `map()` are how you rework them char by char.",
        "~~~ts",
        "'lain'.split('').reverse().join('');  // 'nial'",
        "~~~",
        "## Normalize before comparing",
        "**WHAT** — flatten case and strip noise first: `.toLowerCase()` plus `.replace(/[^a-z0-9]/g, '')`.",
        "**WHY** — a palindrome or anagram test should ignore capitals and punctuation, so 'Lain' and 'lain' must look identical.",
        "~~~ts",
        "const clean = s.toLowerCase().replace(/[^a-z0-9]/g, '');",
        "const isPal = clean === clean.split('').reverse().join('');",
        "~~~",
        "> WARNING — compare the NORMALIZED strings, not the raw ones — otherwise spacing or case quietly breaks the match.",
        "## Counting words",
        "**WHAT** — `s.trim().split(/\\s+/)` cuts on runs of whitespace; its `.length` is the word count.",
        "**WHY** — `trim()` drops edge spaces and `\\s+` collapses double spaces, so messy input still counts correctly.",
        "~~~ts",
        "const words = s.trim() === '' ? 0 : s.trim().split(/\\s+/).length;",
        "~~~",
        "> INTEL — guard the empty string: `''.split(/\\s+/)` returns `['']` (length 1), so handle '' separately as 0 words."
      ),
      exercises: [
        {
