/* Assemble _verify/lua-gen/jobs/*.json into js/curriculum-lua-pack-1.js.
   Lua exercises are test-based (native assert) like the JS track — no
   computed `expected`. The real engine gate is _verify/verify-lua.js (wasmoon);
   author agents self-test via _verify/lua-gen/run.js. This emitter only does
   id-collision checks + structural emit (every string via JSON.stringify).

   Job shapes (one per sector):
     new:  { "type":"new",  "module":{id,code,title,subtitle,theory}, "exercises":[...] }
     fill: { "type":"fill", "modId":"luam01-core", "exercises":[...] }
   Each exercise: { id,title,kind('function'|'script'),difficulty,xp,brief,prompt,
                    starter,solution,tests:[{name,code}],hint,lore }   (Lua source strings) */
const fs = require("fs");
const path = require("path");
const GEN = __dirname, ROOT = path.join(GEN, "..", ".."), JS = path.join(ROOT, "js"), JOBS = path.join(GEN, "jobs");
const TRACK = "lua", PREFIX = "lua", PACK = "curriculum-lua-pack-1.js";

function S(v) { return JSON.stringify(v == null ? "" : v); }
function existingIds() {
  const set = new Set();
  fs.readdirSync(JS).filter((f) => new RegExp("^curriculum-" + TRACK + "(\\.js|-pack-\\d+\\.js)$").test(f)).forEach((f) => {
...
const out = [
  "/* ============================================================",
  "   " + PACK + " — LUA expansion (right-size to ~8 sectors).",
